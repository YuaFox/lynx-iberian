 ...
 */
const t$1 = window, e$4 = t$1.ShadowRoot && (void 0 === t$1.ShadyCSS || t$1.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, s$3 = Symbol(), n$4 = /* @__PURE__ */ new WeakMap();
class o$3 {
...
const r$2 = (t2) => new o$3("string" == typeof t2 ? t2 : t2 + "", void 0, s$3), i$2 = (t2, ...e2) => {
...
d$1.finalized = true, d$1.elementProperties = /* @__PURE__ */ new Map(), d$1.elementStyles = [], d$1.shadowRootOptions = { mode: "open" }, null == o$2 || o$2({ ReactiveElement: d$1 }), (null !== (s$2 = e$3.reactiveElementVersions) && void 0 !== s$2 ? s$2 : e$3.reactiveElementVersions = []).push("1.6.1");
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
var t;
const i$1 = window, s$1 = i$1.trustedTypes, e$2 = s$1 ? s$1.createPolicy("lit-html", { createHTML: (t2) => t2 }) : void 0, o$1 = "$lit$", n$2 = `lit$${(Math.random() + "").slice(9)}$`, l$1 = "?" + n$2, h = `<${l$1}>`, r = document, d = () => r.createComment(""), u = (t2) => null === t2 || "object" != typeof t2 && "function" != typeof t2, c = Array.isArray, v = (t2) => c(t2) || "function" == typeof (null == t2 ? void 0 : t2[Symbol.iterator]), a = "[ 	\n\f\r]", f = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, _ = /-->/g, m = />/g, p = RegExp(`>|${a}(?:([^\\s"'>=/]+)(${a}*=${a}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), g = /'/g, $ = /"/g, y = /^(?:script|style|textarea|title)$/i, w = (t2) => (i2, ...s2) => ({ _$litType$: t2, strings: i2, values: s2 }), x = w(1), T = Symbol.for("lit-noChange"), A = Symbol.for("lit-nothing"), E = /* @__PURE__ */ new WeakMap(), C = r.createTreeWalker(r, 129, null, false), P = (t2, i2) => {
  ...
  for (let i3 = 0; i3 < s2; i3++) {
    ...
    d2 += u2 === f ? s3 + h : v2 >= 0 ? (l2.push(e2), s3.slice(0, v2) + o$1 + s3.slice(v2) + n$2 + w2) : s3 + n$2 + (-2 === v2 ? (l2.push(void 0), i3) : w2);
  }
      ...
      if (1 === h2.nodeType) {
        if (h2.hasAttributes()) {
          ...
          for (const i3 of h2.getAttributeNames())
            if (i3.endsWith(o$1) || i3.startsWith(n$2)) {
              const s2 = f2[u2++];
              if (t3.push(i3), void 0 !== s2) {
                const t4 = h2.getAttribute(s2.toLowerCase() + o$1).split(n$2), i4 = /([.?@])?(.*)/.exec(s2);
...
let LynxBucket = class extends s {
  ...
  _delete() {
    if (confirm(`Delete bucket ${this.name}?`)) {
      axios$1.delete(`/api/v1/bucket/${this.name}`).then(function() {
...
let LynxViewBuckets = class extends LynxView {
  ...
  _input_changed(e2) {
    this.form_new_bucket_name = e2 == null ? void 0 : e2.target.value;
  }
  _createBucket() {
    const self2 = this;
    if (this.form_new_bucket_name === "")
      return alert("Name cannot be blank.");
    const name2 = this.form_new_bucket_name;
    this.form_new_bucket_name = "";
    axios$1.post("/api/v1/bucket", null, {
      params: {
        name: name2
      }
    }).then(function() {
      alert(`Bucket ${name2} created!`);
      self2._query();
    });
  }
  _query() {
    ...
    axios$1.get("/api/v1/bucket", {
      ...
      self2.buckets = response.data;
      self2.bucket_items = [];
      this.buckets.map((bucket) => {
        let e2 = new LynxBucket();
        e2.name = bucket.name;
        self2.bucket_items.push(e2);
      });
    });
  }
  firstUpdated() {
